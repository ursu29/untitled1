...
const injectExtensions = () => {
  ...
  const foundCollapses = {
    ...
    replace: text => {
      ...
      while ((resTitlesList = titlesRegexp.exec(innerString)) !== null) {
        titles.push(resTitlesList?.[0] || '')
      }
      while ((resBodiesList = bodiesRegexp.exec(innerString)) !== null) {
        bodies.push(resBodiesList?.[0] || '')
      }
  ...
  const foundBlocks = {
    ...
    regex: /BLOCK({.*})?\([^]*?\)BLOCK/g,
    replace: text => {
      const color = /(?<=BLOCK{).*?(?=})/g.exec(text)?.[0]
      const body = /(?<=BLOCK({.*})?\(\s*)[^]*?(?=\s*\)BLOCK)/g.exec(text)?.[0]
      return `<div class='md-injected-block' data-color='${color}'>${body}</div>`
    },
  }
  const foundSpaces = {
    type: 'lang',
    ...
    replace: text => {
      ...
      return `<div class='md-injected-space'>${space}</div>`
    },
  }
