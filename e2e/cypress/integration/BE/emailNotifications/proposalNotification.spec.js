import {
  attachMatrixToEmployee, createMatrixProposal, deleteMatrixProposal,
...
const text = {
  ...
  body: `employee test.employee@syncretis.com left a new proposal`,
  proposal: `proposal: ${todaysDate}`
}

describe('send notification when create proposal', () => {
  let matrixData, createProposalId, emailData
  ...
  it('create new proposal', function() {
    cy.post(getAllEmployeeMatrices(this.userId)).then(req => {
      ...
      cy.post(createMatrixProposal(id, matrixData.id, text.proposal))
        .then(req => {
  ...
  it(`successfully sent  message to owner`, () => {
    cy.setToken('manager')
